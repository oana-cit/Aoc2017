...
def findEvenlyDivisible(line):
    global theirSum
    
    for i in range(len(line)):
        for j in range(i + 1, len(line)):
            if line[i] % line[j] == 0:
                theirSum += line[i] / line[j]
                print("Add: {} % {}".format(line[i], line[j]))
                return   
            if line[j] % line[i] == 0:
                theirSum += line[j] / line[i]
                print("Add: {} % {}".format(line[j], line[i]))
                return
...
#     9 4 7 3
#     3 8 6 5'''
    allNumbers = '''86    440    233    83    393    420    228    491    159    13    110    135    97    238    92    396
...
3111    2857    2312    3230    149    3082    408    1148    2428    134    147    620    128    157    492    2879'''
    ...
    for line in allNumbers.splitlines():
        findEvenlyDivisible(list(map(int, line.split())))

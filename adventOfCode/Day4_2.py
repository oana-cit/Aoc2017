...
def checkAnagram(usedWords):
    for i in range(len(usedWords)):
        for j in range(i + 1, len(usedWords)):
            firstGo = Counter(list(usedWords[i]))
            secondGo = Counter(list(usedWords[j]))
            print("compare: {} and {} => {}".format(firstGo, secondGo, firstGo == secondGo))
            if firstGo == secondGo:
                return True
    return False

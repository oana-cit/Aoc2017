...
if __name__ == '__main__':
    ...
    totalOk = 0
    allPassphrases = '''abcde fghij
    abcde xyz ecdab
    a ab abc abd abf abj
    iiii oiii ooii oooi oooo
    oiii ioii iioi iiio'''
#     allPassphrases = '''vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
# xzvfbf fopmfxu mvftgr mfupoxf coyhof talcc vpkslo'''
    ...
    for crtPassphrase in allPassphrases.splitlines():
        usedWords = crtPassphrase.split()
        lenUsedWords = len(usedWords)
        print("usedWords: {}".format(usedWords))
        reversedWords = list(map(lambda x: "".join(reversed(x)), usedWords))
        print("usedWords: {}".format(usedWords))
        print("reversedWords: {}".format(reversedWords))
        allWords = set(usedWords + reversedWords)
        print("allWords: {}".format(allWords))
        if lenUsedWords == len(allWords):
            totalOk += 1
      
    print("Total ok: {}".format(totalOk))